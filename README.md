...
- `assets/coast-source.glb` is the generated geometry before UV unwrapping and lighting. `src/experience/buildCoast.js` and `coastLayout.js` author it deterministically (29 material groups, 26,524 triangles).
...
The runtime lighting textures total 7.40 MB, the landscape 1.77 MB, and the featured models 3.54 MB (decimal MB). Original exports and PNGs stay available locally and are excluded from the production build. Raw EXR bake intermediates live in `assets/lighting/` and are excluded from Git and the production build.

## Rebuild the landscape

1. Run `npm run models:coast` to regenerate `assets/coast-source.glb`.
